...
export function RegisterCard() {
  ...
  const updateProviderType = (index: number, type: string) => {
    updateProvider(index, {
      ...
      ...(type === "cloudflare_temp_email" ? { api_base: "", admin_password: "", site_password: "", domain: [] } : {}),
                      ...
                      {type === "cloudmail_gen" || type === "cloudflare_temp_email" || type === "moemail" || type === "inbucket" || type === "yyds_mail" || type === "ddg_mail" ? (
                        <>
                          ...
                          {type === "cloudflare_temp_email" ? (
                            <div className="space-y-2">
                              <label className="text-sm text-stone-700">Site Password</label>
                              <Input value={String(provider.site_password || "")} onChange={(event) => updateProvider(index, { site_password: event.target.value })} className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} placeholder="私有站点访问密码，可选" />
                            </div>
                          ) : null}
                        </>
                      ) : null}
